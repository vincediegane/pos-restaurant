...
- `models/restaurant.py` — `resto.restaurant` (the single restaurant record) and a `pos.config` extension (`resto_lock_cashier_to_user`) that restricts the POS cashier-selection dropdown to the logged-in user unless they're a POS manager. `resto.restaurant.logo` is a non-stored compute+inverse mirror of `res.company.logo` — the *only* logo actually used anywhere (POS receipts, PDF reports, dashboard exports all read `res.company.logo` directly); it's exposed here because editing it via Odoo's own Settings > Companies screen isn't something a restaurant manager would find.
- `static/src/pos/` — POS (point_of_sale) frontend extensions, loaded into the `point_of_sale._assets_pos` bundle (declared in `__manifest__.py`, following the same `'restaurant_starter/static/src/pos/**/*'` glob pattern `pos_restaurant` itself uses). `order_receipt_qr.js` patches `Order.prototype.export_for_printing` (via `@web/core/utils/patch`) to add a `resto_qr_code` field built from Odoo's own barcode/QR report route (`qrCodeSrc` from `@point_of_sale/utils`, i.e. `/report/barcode/QR/...` — no third-party service, works offline on the local network); `order_receipt_qr.xml` inherits `point_of_sale.OrderReceipt` (`t-inherit-mode="extension"`) to print it under the receipt footer.
...
- Extending the POS frontend (OWL) means contributing files to the `point_of_sale._assets_pos` bundle from your own manifest — a module doesn't need to touch `point_of_sale`'s own manifest, just glob its own `static/src/pos/**/*` into that bundle name (see `pos_restaurant`'s manifest for the precedent). Class methods are extended via `patch()` from `@web/core/utils/patch`; OWL XML templates via `<t t-inherit="..." t-inherit-mode="extension">` with an xpath, same idea as backend view inheritance. Odoo doesn't validate these server-side beyond well-formed XML — `env["ir.asset"]._get_asset_paths(bundle=..., assets_params={})` in `odoo shell` confirms a file made it into the bundle, but a bad xpath or patch target only surfaces as a browser console error at POS boot; there's no way to check that from `odoo shell` alone.
