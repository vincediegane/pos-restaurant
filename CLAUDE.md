...
```
Coverage is a starting point, not exhaustive: role→group sync (`tests/test_res_users.py`), the per-role menu visibility (`tests/test_menu_access.py`), and `resto.restaurant`/cashier-lock behavior (`tests/test_restaurant.py`). Nothing exercises `controllers/dashboard.py` at the HTTP layer or the dashboard's JS.
